...
interface BConvertProps {
    func: () => void; // Ensure correct typing
  }
...
export default function BConvert({ func }: BConvertProps) {
  ...
  return (
    ...
    >
      <div className="absolute flex items-center justify-center w-10 h-10 max-h-32 max-w-32 top-[-3rem] right-0 bg-[#FF2222] rounded-md cursor-pointer hover:shadow-[3px_3px_0px_white] hover:translate-x-[-2px] hover:translate-y-[-2px] transition-all duration-75 active:shadow-[0px_0px_0px_white] active:translate-x-0 active:translate-y-0"
      onClick={() => {
        setDecimal(0);
        func();
      }}>
      ...
      <span className="flex gap-4">
        ...
        className="text-white font-neueMachina p-3 w-full focus:outline-none bg-black rounded-md placeholder:text-gray-300/50 placeholder:font-neueMachina"
        />
      </span>
      <span className="flex gap-4">
        <h1 className="text-black text-3xl font-neueMachina">Binary</h1>
        <div className="p-3 bg-black text-white font-neueMachina h-full w-full ml-7 rounded-md"> {binary === "NaN" ? <span className="text-white font-neueMachina">0</span> : binary } </div>
      </span>
    </motion.div>
    </>
  );
